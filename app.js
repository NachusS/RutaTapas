...
// Perfil (v7.0)
const PROFILE_KEY = "RT_PROFILE_v1";
...
window.initMap = async function initMap(){
  ...
  if(routes.routes.length===0){
    ...
    try{
      ...
      console.error(e);
      showDiag('Error cargando la ruta seleccionada. Revisa los JSON en data/.');
    }
  }
...
function initProfileUI(){
  ...
  const createScreen = document.querySelector("#authCreate");
  const welcomeScreen = document.querySelector("#authWelcome");
  if(!overlay || !appShell || !createScreen || !welcomeScreen) return;
  ...
  function showCreate(){
    ...
    buttons.forEach((b,i)=> b.classList.toggle("is-selected", i===0));
    overlay.dataset.selectedAvatar = buttons[0]?.getAttribute("data-avatar") || "";
  }
  ...
  function showWelcome(p){
    ...
    const name = (p?.name || "").trim() || "Amigo";
    const avatar = p?.avatar || "";
    const img = document.querySelector("#welcomeAvatarImg");
    if(img) img.src = avatar || "assets/avatars/avatar-oliva.svg";

    const title = document.querySelector("#welcomeTitle");
    if(title) title.textContent = `¡Hola de nuevo, ${name}!`;
  }
  ...
  function enterApp(){
    ...
    // Si el mapa se creó mientras el contenedor estaba oculto, forzamos resize/repaint
    try{
      if(window.google && google.maps && google.maps.event && state && state.map){
  ...
  avatarButtons.forEach(btn=>{
    btn.addEventListener("click", ()=>{
      ...
      if(url) overlay.dataset.selectedAvatar = url;

      // Reset photo preview if user goes back to avatar
      const photoBtn = document.querySelector(".photo-btn");
      if(photoBtn){
        photoBtn.style.backgroundImage = "";
        const inner = photoBtn.querySelector(".photo-btn-inner");
        if(inner) inner.style.display = "";
        photoBtn.style.borderStyle = "dashed";
        photoBtn.style.borderColor = "rgba(255,255,255,.30)";
      }
  ...
  if(photoInput){
    photoInput.addEventListener("change", ()=>{
      ...
      reader.onload = ()=>{
        ...
        overlay.dataset.selectedAvatar = dataUrl;

        if(photoBtn){
          photoBtn.style.backgroundImage = `url('${dataUrl}')`;
          photoBtn.style.backgroundSize = "cover";
          photoBtn.style.backgroundPosition = "center";
          const inner = photoBtn.querySelector(".photo-btn-inner");
          if(inner) inner.style.display = "none";
          photoBtn.style.borderStyle = "solid";
          photoBtn.style.borderColor = "rgba(236,109,19,.55)";
        }
  ...
  // Create profile
  const createBtn = document.querySelector("#createProfileBtn");
  if(createBtn){
    createBtn.addEventListener("click", ()=>{
      const name = (document.querySelector("#profileName")?.value || "").trim();
      if(!name){
        // existing helper in app.js
        showDiag("Escribe tu nombre para crear el perfil.");
        document.querySelector("#profileName")?.focus();
        return;
      }
      let avatar = overlay.dataset.selectedAvatar || "";
      if(!avatar){
        const sel = document.querySelector(".avatar-btn.is-selected");
        avatar = sel ? (sel.getAttribute("data-avatar")||"") : "";
      }
      if(!avatar) avatar = "assets/avatars/avatar-oliva.svg";

      setProfile({ name, avatar });
      showWelcome(getProfile());
    });
  }

  // Continue
  document.querySelector("#continueBtn")?.addEventListener("click", enterApp);

  // Switch account
  document.querySelector("#switchAccountBtn")?.addEventListener("click", ()=>{
    clearProfile();
    overlay.dataset.selectedAvatar = "";
